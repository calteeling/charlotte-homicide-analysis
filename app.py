...
# Inject CSS to remove excess padding and overflow scroll
st.markdown("""
    <style>
...
if page == 'Heatmap':
    ...
    if month != "All":
        month_number = [num for num, label in month_labels.items() if label == month][0]
        filtered_df = filtered_df[filtered_df["Month"] == month_number]
    if weekday != "All":
        weekday_number = [num for num, label in weekday_labels.items() if label == weekday][0]
        filtered_df = filtered_df[filtered_df["Weekday"] == weekday_number]
    if weapon != "All":
...
#Tab 2 
    ...
    if lat is not None and lon is not None:
        ...
        def predict_weapon(df_input):
            return model.predict(df_input)
        if st.button('Predict Weapon'):
            pred = predict_weapon(input_df)[0]
...
#Tab 3
    ...
    if reset_view or filtered_df.empty:
        map_center = [35.2271, -80.8431]
    else:
        try:
            map_center = filtered_df[['Latitude', 'Longitude']].mean().values.tolist()
